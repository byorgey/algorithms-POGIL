  ...
  \begin{algorithm}[H]
    \begin{algorithmic}[1]
      ...
      \For{each edge $e \in E$}
        \If{$e$ does not make a cycle with other edges already in $T$}
          \State Add $e$ to $T$
        \EndIf
      \EndFor
    \end{algorithmic}
...
\item The way the algorithm is written in \pref{model:kruskal}, one
  ...
  stop the loop early? \marginnote{\emph{Hint: use the fact that the result must
  be a spanning tree.}}
...
\begin{proof}
  ...
  $G$ which does \textbf{not} contain the edge $e$; we will show that
  $M$ is not a minimum spanning tree.  Since $M$ is a
...
\begin{thm}
  Kruskal's Algorithm is correct (when all edges have distinct
  weights).\marginnote{It is not too much harder to show that Kruskal's Algorithm works even
when there are duplicate weights.}
\end{thm}
...
\begin{proof}
  ...
  Algorithm wouldn't choose it. The edge $e$ therefore crosses the cut
  $(S,T)$. No other edges which have been picked previously cross the
