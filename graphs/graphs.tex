...
\begin{model}{Graphs and graph terms}{graphs}
  ...
  \vspace{1em}
  \begin{defn}
    A (simple) \term{graph} $G = (V,E)$ is a set of \term{vertices}
    $V$ (singular \term{vertex}) together with a set $E$ of
    \term{edges}, where each edge consists of a set of two vertices.
  \end{defn}
...
\item How many vertices does the numeric graph have?  How many edges?
  ...
  \begin{marginfigure}
    \begin{center}
      \includegraphics[width=1in]{sad-dog}
...
\item What do you think is the definition of a tree?  You should use
  ...
  definition.\marginnote{\textbf{Warning}---a tree graph is not quite the same
