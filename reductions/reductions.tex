...
\begin{model*}{Independent sets}{IS}
  ...
  \end{center}
\end{model*}
\marginnote{Think of edges as hallways in an art museum and $C$ as the
  set of locations where we are going to put some guards.  Then an
  independent set means no two guards can see each other; a vertex
  cover means every hallway is watched by at least one guard.}
